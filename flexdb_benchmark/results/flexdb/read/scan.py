...
for j in range(len(sizes)):
    ...
    ax[j].set_ylim(0, 0.9)
    ax[j].set_yticks([0, 0.2, 0.4, 0.6, 0.8])
    ...
    if j == 1:
        ax[j].set_xlabel("Scan Length", fontsize=fontsize)
    if j == 0:
...
# plot
fig.savefig("flexdb-scan.pdf", bbox_inches="tight", pad_inches=0.03, format='pdf')
plt.close(fig)
