...
fig = plt.figure(constrained_layout=True, figsize=(3.5, 2.65))
fontsize = 15
ticksize = 12
barwidth= 0.3
    ...
    for i in range(len(targets)):
        ...
        if t[0] == "fdb":
            for _p in range(len(ax.patches)):
                p = ax.patches[_p]
                if (_p == 3):
                    ax.annotate("{:.2f}".format(baseline[_p])+"M", (p.get_x()-0.385, p.get_height() * 1.05), fontsize=fontsize-2)
                else:
                    if (_p == 5):
                        ax.annotate("{:.0f}".format(baseline[_p]*1e3)+"K", (p.get_x()-0.42, p.get_height() * 1.05), fontsize=fontsize-2)
                    else:
                        ax.annotate("{:.0f}".format(baseline[_p]*1e3)+"K", (p.get_x()-0.32, p.get_height() * 1.05), fontsize=fontsize-2)
...
handles = handles[::-1][1:]
labels = labels[::-1][1:]
fig.legend(handles, labels, loc='upper center', fontsize=fontsize, ncol=3, bbox_to_anchor=(0.17, 1.08), handletextpad=0.4, borderpad=0.25, columnspacing=1.8)
# plot
fig.savefig("flexdb-ycsb-oc.pdf", bbox_inches="tight", pad_inches=0.03, format='pdf')
plt.close(fig)
